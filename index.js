...
const questions = [{
    ...
    t_employees : {
	prompt: () => utils.recursivePromptAsync(forms.addEmployees,
						 (answers, acc) => [...acc, answers],
						 answers => answers.addEmployee,
						 []),
	serialization: employees => employees
    ...
    t_trucks : {
	prompt: () => utils.recursivePromptAsync(forms.addTrucks,
						 (answers, acc) => [...acc, answers],
						 answers => answers.addTruck,
						 []),
	serialization: trucks => trucks
	    .reduce((acc, t) => acc += t.t_truckType + ':' + t.t_truckVolume + ';' , '')
	    .slice(0, -1),
...
async function ask(client, forms, backup) {
    const input = await Promise.reduce(forms, async (acc, form) => {
...
async function main(questions){
	...
	const answers = await ask(client, forms.toAsk, utils.inflateBackup(backup, forms.backedUp))
